...
async fn fix_transfer_key() {
    ...
    let query = "select name_hash from ansb.ans_name where transfer_key is null";
    ...
    for row in rows {
        ...
        db_client.execute("update ansb.ans_name set transfer_key=$1 where name_hash=$2",
...
async fn index_data(block: &Block<N>) {
    ...
    db_trans.execute("INSERT INTO ansb.block (height, block_hash, previous_hash, timestamp) VALUES ($1, $2,$3, $4) ON CONFLICT (height) DO NOTHING",
...
async fn register(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        let query = "SELECT full_name FROM ansb.ans_name WHERE name_hash=$1 limit 1";
        ...
        db_trans.execute("INSERT INTO ansb.ans_name (name_hash, transfer_key, name, parent, resolver, full_name, block_height, transaction_id, transition_id) \
...
async fn register_tld(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("INSERT INTO ansb.ans_nft_owner (name_hash, address, block_height, transaction_id, transition_id) \
...
async fn convert_public_to_private(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("DELETE from ansb.ans_nft_owner WHERE name_hash=$1", &[&name_hash]).await.unwrap();

        let version = 2;
        db_trans.execute("INSERT INTO ansb.ans_name_version (name_hash, version, block_height, transaction_id, transition_id) \
                                    VALUES ($1, $2,$3, $4, $5) ON CONFLICT (name_hash) DO UPDATE SET version = ansb.ans_name_version.version + 1, block_height=$3, transaction_id=$4, transition_id=$5",
                         &[&name_hash, &version, &(block.height() as i64), &transaction.id().to_string(), &transition.id().to_string()]
        ).await.unwrap();
        db_trans.execute("DELETE from ansb.ans_primary_name WHERE name_hash=$1 AND address=$2", &[&name_hash, &owner]).await.unwrap();
...
async fn transfer_public(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        let query = "SELECT address FROM ansb.ans_nft_owner WHERE name_hash=$1 limit 1";
        let query = db_trans.prepare(&query).await.unwrap();
...
async fn set_primary_name(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("INSERT INTO ansb.ans_primary_name (name_hash, address, block_height, transaction_id, transition_id) \
...
async fn unset_primary_name(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("DELETE from ansb.ans_primary_name WHERE address=$1", &[&owner]).await.unwrap();
...
async fn set_resolver(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("UPDATE ansb.ans_name set resolver=$1  WHERE name_hash=$2 ",
                         &[&resolver, &name_hash]
        ).await.unwrap();
...
async fn set_resolver_record(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        let mut version = 1;
        let query = "SELECT version FROM ansb.ans_name_version WHERE name_hash=$1 limit 1";
        let query = db_trans.prepare(&query).await.unwrap();
        let rows = db_trans.query(&query, &[&name_hash]).await.unwrap();
        if !rows.is_empty() {
        ...
        db_trans.execute("INSERT INTO ansb.ans_resolver (name_hash, category, version, name, block_height, transaction_id, transition_id) \
...
async fn unset_resolver_record(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("DELETE from ansb.ans_resolver where name_hash=$1 and category=$2 and version=$3 ",
...
async fn burn(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("DELETE from ansb.ans_name WHERE name_hash=$1", &[&name_hash]).await.unwrap();
...
async fn transfer_credits(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("INSERT INTO ansb.domain_credits (transfer_key, amount, block_height, transaction_id, transition_id) \
                                    VALUES ($1, $2,$3, $4, $5) ON CONFLICT (transfer_key) DO UPDATE SET amount = ansb.domain_credits.amount + $2, block_height=$3, transaction_id=$4, transition_id=$5",
...
async fn claim_credits(db_trans: &tokio_postgres::Transaction<'_>, block: &Block<N>, transaction: &Transaction<N>, transition: &Transition<N>) {
    ...
    if let Some(may_future) = outs_last.future() {
        ...
        db_trans.execute("UPDATE ansb.domain_credits SET amount = ansb.domain_credits.amount - $2, block_height=$3, transaction_id=$4, transition_id=$5 where transfer_key=$1",
                         &[&transfer_key, &(amount as i64), &(block.height() as i64), &transaction.id().to_string(), &transition.id().to_string()]
        ).await.unwrap();
